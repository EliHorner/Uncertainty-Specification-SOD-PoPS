...
for (i in setupList){
  ...
  sampleslist <- filelist[1:samples]
...
sobolTotalOrderRast <- function(rastersList, uMat, uSource, totalVar){
  ...
  return(1 - (numerator/ totalVar))
}


#Run Summarized Sobol Analyses (whole study area) Lump Method
SFOvalsLump <- c(rep(0, num_sources))
STOvalsLump <- c(rep(0, num_sources))

for(i in 1:num_sources){
  SFOvalsLump[i] <- sobolFirstOrder(ValsStack$sum, out_u_mat, i)
  STOvalsLump[i] <- sobolTotalOrder(ValsStack$sum, out_u_mat, i, var(ValsStack$sum))
}

write.csv(SFOvalsLump, paste0(outpath, 'Lump2k/SFOVals.csv'))
write.csv(STOvalsLump, paste0(outpath, 'Lump2k/STOVals.csv'))

#Run Sobol Analyses on Rasters (at cell level) Lump Method
for(i in 1:num_sources){
  assign(paste0('SFO_lump_', source_list[i]), sobolFirstOrderRast(RastsStack, out_u_mat, i, sampleVar))
  writeRaster(get(paste0('SFO_lump_', source_list[i])), paste0(outpath, 'Lump2k/', paste0('SFO_lump_', source_list[i], '.tif')))
}

for(i in 1:num_sources){
  assign(paste0('STO_lump_', source_list[i]), sobolTotalOrderRast(RastsStack, out_u_mat, i, sampleVar))
  writeRaster(get(paste0('STO_lump_', source_list[i])), paste0(outpath, 'Lump2k/', paste0('STO_lump_', source_list[i], '.tif')))
}
