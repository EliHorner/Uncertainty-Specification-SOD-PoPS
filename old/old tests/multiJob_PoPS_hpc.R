...
if (argslen < 1) stop('Error: Takes 1 argument, case to be run')

cat("There were ",argslen," arguments.\n")
cat("Arguments were ",args,"\n")

#Define which case to run by the command line argument
which_case=as.integer(args[1])
cases<- c('all', 'host', 'ic', 'par', 'nohost', 'noic', 'nopar', 'none')
case<- cases[which_case]
cat("The case being run is ",case,".\n",sep="")
...
uncertRunsSobol(case = case, years = 1)
